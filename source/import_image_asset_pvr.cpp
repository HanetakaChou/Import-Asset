...
//--------------------------------------------------------------------------------------
extern bool import_image_asset_header_from_input_stream_pvr(import_asset_input_stream *input_stream, BRX_SAMPLED_ASSET_IMAGE_IMPORT_HEADER *image_asset_header, size_t *image_asset_data_offset)
{
    assert(image_asset_header != NULL);
    assert(image_asset_data_offset != NULL);

    if (-1 == input_stream->seek(0, IMPORT_ASSET_INPUT_STREAM_SEEK_SET))
    {
        return false;
    }
    ...
    // TODO: handle metadata
    {
        ...
        while (metaDataRead < header.metaDataSize)
        {
            ...
            if (-1 == input_stream->seek(metadata._dataSize, IMPORT_ASSET_INPUT_STREAM_SEEK_CUR))
            {
                return false;
            }
    ...
    if (1U == header.depth)
    {
        if (header.height > 1U)
        {
            image_asset_header->type = BRX_SAMPLED_ASSET_IMAGE_TYPE_2D;
        }
        else if (1U == header.height)
        {
            image_asset_header->type = BRX_SAMPLED_ASSET_IMAGE_TYPE_1D;
        }
        else
        {
            return false;
    ...
    {
        image_asset_header->type = BRX_SAMPLED_ASSET_IMAGE_TYPE_3D;
    }
    else
    {
        return false;
    ...
    if (0U == Pvr_GetPixelFormatPartHigh(header.pixelFormat))
    {
        ...
        {
            image_asset_header->format = (Pvr_ColorSpace_lRGB == header.colorSpace) ? BRX_SAMPLED_ASSET_IMAGE_FORMAT_BC7_UNORM_BLOCK : ((Pvr_ColorSpace_sRGB == header.colorSpace) ? static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1) : static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1));
        }
        break;
        case Pvr_PixelTypeID_ASTC_4x4:
        {
            image_asset_header->format = (Pvr_ColorSpace_lRGB == header.colorSpace) ? BRX_SAMPLED_ASSET_IMAGE_FORMAT_ASTC_4x4_UNORM_BLOCK : ((Pvr_ColorSpace_sRGB == header.colorSpace) ? static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1) : static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1));
        }
        break;
        default:
    ...
    else
    {
        ...
        {
            image_asset_header->format = (Pvr_ChannelType_UnsignedByteNorm == header.channelType) ? ((Pvr_ColorSpace_lRGB == header.colorSpace) ? BRX_SAMPLED_ASSET_IMAGE_FORMAT_R8G8B8A8_UNORM : static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1)) : static_cast<BRX_SAMPLED_ASSET_IMAGE_FORMAT>(-1);
        }
        break;
        default:
            // TODO: support more format
...
extern bool import_image_asset_data_from_input_stream_pvr(import_asset_input_stream *input_stream, BRX_SAMPLED_ASSET_IMAGE_IMPORT_HEADER const *image_asset_header, size_t image_asset_data_offset, void *staging_upload_buffer_base, size_t subresource_count, BRX_SAMPLED_ASSET_IMAGE_IMPORT_SUBRESOURCE_MEMCPY_DEST const *subresource_memcpy_dests)
{
#ifndef NDEBUG
    BRX_SAMPLED_ASSET_IMAGE_IMPORT_HEADER image_asset_header_for_validate;
    size_t image_asset_data_offset_for_validate;
    if (!import_image_asset_header_from_input_stream_pvr(input_stream, &image_asset_header_for_validate, &image_asset_data_offset_for_validate))
    {
        return false;
    }
    ...
    size_t const numberOfResources = ((BRX_SAMPLED_ASSET_IMAGE_TYPE_3D == image_asset_header->type) ? static_cast<size_t>(1U) : static_cast<size_t>(image_asset_header->array_layers)) * static_cast<size_t>(image_asset_header->mip_levels) * static_cast<size_t>(numberOfPlanes);
    ...
    for (uint32_t mipMap = 0; mipMap < image_asset_header->mip_levels; ++mipMap)
    {
        ...
        for (uint32_t arrayIndex = 0U; arrayIndex < image_asset_header->array_layers; ++arrayIndex)
        {
            ...
            for (uint32_t planeIndex = 0; planeIndex < 1U; ++planeIndex)
            {
                ...
                {
                    assert(input_stream->seek(0, IMPORT_ASSET_INPUT_STREAM_SEEK_CUR) == inputSkipBytes);
                ...
                else if (inputRowSize == subresource_memcpy_dests[dstSubresource].output_row_pitch)
                {
                    ...
                    {
                        assert(input_stream->seek(0, IMPORT_ASSET_INPUT_STREAM_SEEK_CUR) == (inputSkipBytes + inputSliceSize * z));
                ...
                else
                {
                    ...
                    for (size_t z = 0; z < inputNumSlices; ++z)
                    {
                        ...
                        {
                            assert(input_stream->seek(0, IMPORT_ASSET_INPUT_STREAM_SEEK_CUR) == (inputSkipBytes + inputSliceSize * z + inputRowSize * y));
