...
{
	void *new_unwrapped_input_stream_factory_base = import_asset_malloc(sizeof(import_asset_input_stream_factory_file), alignof(import_asset_input_stream_factory_file));
...
extern void import_asset_destroy_file_input_stream_factory(import_asset_input_stream_factory *wrapped_input_stream_factory)
{
	...
	import_asset_free(delete_unwrapped_input_stream_factory);
}
...
import_asset_input_stream *import_asset_input_stream_factory_file::create_instance(char const *file_name)
{
	...
	if (-1 != file)
	{
		void *new_unwrapped_input_stream_base = import_asset_malloc(sizeof(import_asset_file_input_stream), alignof(import_asset_file_input_stream));
...
void import_asset_input_stream_factory_file::destory_instance(import_asset_input_stream *wrapped_input_stream)
{
	...
	import_asset_free(delete_unwrapped_input_stream);
}
...
import_asset_input_stream *import_asset_input_stream_factory_file::create_instance(char const *file_name)
{
	...
	import_asset_wstring file_name_utf16;
	{
		// llvm::convertUTF8ToUTF16String

		assert(file_name_utf16.empty());

		import_asset_string file_name_utf8 = file_name;

		if (!file_name_utf8.empty())
		{
			llvm::UTF8 const *src = reinterpret_cast<const llvm::UTF8 *>(file_name_utf8.data());
			llvm::UTF8 const *src_end = reinterpret_cast<const llvm::UTF8 *>(file_name_utf8.data() + file_name_utf8.size());
			...
			file_name_utf16.resize(file_name_utf8.size() + 1U);
			llvm::UTF16 *dst = reinterpret_cast<llvm::UTF16 *>(&file_name_utf16[0]);
			llvm::UTF16 *dst_end = dst + file_name_utf16.size();

			llvm::ConversionResult CR = llvm::ConvertUTF8toUTF16(&src, src_end, &dst, dst_end, llvm::strictConversion);
			assert(CR != llvm::targetExhausted);

			assert(llvm::conversionOK == CR);

			file_name_utf16.resize(dst - reinterpret_cast<llvm::UTF16 *>(&file_name_utf16[0]));
			// file_name_utf16.push_back(L'0');
			// file_name_utf16.pop_back();
		}
		else
		{
			// file_name_utf16.push_back(L'0');
	...
	HANDLE file = CreateFileW(file_name_utf16.c_str(), FILE_READ_DATA | FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
