...
extern bool import_scene_asset_gltf(std::vector<scene_mesh_data> &out_total_mesh_data, float frame_rate, char const *path)
{
    ...
    {
        out_total_mesh_data.push_back({});
        scene_mesh_data &out_mesh_data = out_total_mesh_data.back();
        ...
        if (!out_mesh_data.m_skinned)
        {
            ...
            for (size_t mesh_instance_index = 0; mesh_instance_index < mesh_instance_count; ++mesh_instance_index)
            {
                ...
                out_mesh_instance_data.push_back({mesh_instance_node_world_transforms[mesh_instance_index]});
            }
        }
        else
        {
            ...
            for (size_t mesh_instance_index = 0; mesh_instance_index < mesh_instance_count; ++mesh_instance_index)
            {
                ...
                if (NULL != skin)
                {
                    ...
                    out_mesh_instance_data.push_back({model_transform});

                    import_gltf_scene_animation_asset(&out_mesh_instance_data.back().m_animation_skeleton, frame_rate, data, skin, animation);
                }
                else
                {
                    ...
                    out_mesh_instance_data.back().m_animation_skeleton.init(1, (max_joint_index + 1));
                    ...
                    {
                        out_mesh_instance_data.back().m_animation_skeleton.set_transform(0, joint_index, quaternion, translation);
                    }
                }
            }
        }
...
#pragma warning(disable : 4996)
                        ...
                        packed_vector.w = static_cast<int32_t>(std::copysign(1.0, tangent_float4[3]));
#if defined(_MSC_VER)
#pragma warning(pop)
#endif
                ...
                scene_mesh_vertex_skinned_binding *const out_vertex_skinned_binding = &subset_data->m_vertex_skinned_binding[vertex_index_offset];
    ...
    if ((*out_max_joint_index) < 0)
    {
        ...
        {
            std::vector<scene_mesh_vertex_skinned_binding> temp_empty;
...
#include <sdkddkver.h>
#include <Windows.h>
#include "../../ConvertUTF/include/ConvertUTF.h"
...
static cgltf_result cgltf_custom_read_file(const struct cgltf_memory_options *memory_options, const struct cgltf_file_options *, const char *path, cgltf_size *size, void **data)
{
    ...
    std::wstring path_utf16;
    {
        ...
        bool res_convert_utf8_To_utf16_string = llvm::convertUTF8ToUTF16String(path_utf8, path_utf16);
        assert(res_convert_utf8_To_utf16_string);
    }

    HANDLE file;
    if (INVALID_HANDLE_VALUE == (file = CreateFileW(path_utf16.c_str(), FILE_READ_DATA | FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)))
    {
    ...
    if (file_size == 0)
    {
        ...
        if (FALSE == res_get_file_size_ex || length.QuadPart < 0)
        {
...
static void *cgltf_custom_alloc(void *, cgltf_size size)
{
    return _aligned_malloc(size, alignof(size_t));
}
