...
#error Unknown Compiler
                ...
                if (NULL != joints_accessor)
                {
                    ...
                    case cgltf_component_type_r_8u:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < joints_accessor->count; ++vertex_index)
                        {
                            ...
                            (*reinterpret_cast<uint64_t *>(&out_vertex_skinned_binding[vertex_index].m_indices_xy)) = packed_vector.v;
                        }
                    }
                    break;
                    case cgltf_component_type_r_16u:
                ...
                else
                {
                    for (size_t vertex_index = 0; vertex_index < vertex_count; ++vertex_index)
                    {
                        out_vertex_skinned_binding[vertex_index].m_indices_xy = 0U;
                        out_vertex_skinned_binding[vertex_index].m_indices_wz = 0U;
                    }
                }
                ...
                if (NULL != weights_accessor)
                {
                    ...
                    case cgltf_component_type_r_8u:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < weights_accessor->count; ++vertex_index)
                        {
                            ...
                            out_vertex_skinned_binding[vertex_index].m_weights = packed_vector_ubyten4.v;
                        }
                    }
                    break;
                    case cgltf_component_type_r_16u:
...
static cgltf_result cgltf_custom_read_file(const struct cgltf_memory_options *memory_options, const struct cgltf_file_options *, const char *path, cgltf_size *size, void **data)
{
    ...
    import_asset_input_stream_file file;
    if (!file.init(path))
    {
...
static void *cgltf_custom_alloc(void *, cgltf_size size)
{
    return aligned_alloc(alignof(size_t), size);
}
