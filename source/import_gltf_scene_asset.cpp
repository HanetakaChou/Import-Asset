...
#include <algorithm>
#include <assert.h>
#include "../shaders/packed_vector.sli"
...
static void import_gltf_scene_mesh_instance_asset(std::vector<cgltf_node const *> &out_mesh_instance_nodes, std::vector<DirectX::XMFLOAT4X4> &out_mesh_instance_node_world_transforms, cgltf_data const *data, cgltf_mesh const *mesh);
...
extern bool import_gltf_scene_asset(std::vector<scene_mesh_data> &out_total_mesh_data, float frame_rate, import_asset_input_stream_factory *input_stream_factory, char const *path)
{
    ...
    for (size_t mesh_index = 0; mesh_index < data->meshes_count; ++mesh_index)
    {
        ...
        std::vector<scene_mesh_instance_data> &out_mesh_instance_data = out_mesh_data.m_instances;
...
static void import_gltf_scene_mesh_asset(scene_mesh_data *out_mesh_data, int32_t *out_max_joint_index, cgltf_data const *data, cgltf_mesh const *mesh)
{
    ...
    for (size_t primitive_index = 0; primitive_index < mesh->primitives_count; ++primitive_index)
    {
        ...
        if (cgltf_primitive_type_triangles == primitive->type)
        {
            ...
            {
                scene_mesh_subset_data *subset_data;
                ...
                scene_mesh_vertex_position_binding *const out_vertex_position_binding = &subset_data->m_vertex_position_binding[vertex_index_offset];
                ...
                scene_mesh_vertex_varying_binding *const out_vertex_varying_binding = &subset_data->m_vertex_varying_binding[vertex_index_offset];
                ...
                if (NULL != normal_accessor)
                {
                    ...
                    for (size_t vertex_index = 0; vertex_index < normal_accessor->count; ++vertex_index)
                    {
                        ...
                        DirectX::XMFLOAT2 const mapped_normal = octahedron_map(*reinterpret_cast<DirectX::XMFLOAT3 const *>(normal_float3));

                        DirectX::PackedVector::XMSHORTN2 packed_vector;
                        DirectX::PackedVector::XMStoreShortN2(&packed_vector, DirectX::XMLoadFloat2(&mapped_normal));
                ...
                if (NULL != tangent_accessor)
                {
                    ...
                    for (size_t vertex_index = 0; vertex_index < tangent_accessor->count; ++vertex_index)
                    {
                        ...
                        out_vertex_varying_binding[vertex_index].m_tangent = FLOAT3_to_R15G15B2_SNORM(octahedron_map(*reinterpret_cast<DirectX::XMFLOAT3 const *>(tangent_float4)), tangent_float4[3]);
                    }
                }
                else
                {
                ...
                assert(vertex_index_offset == subset_data->m_vertex_joint_binding.size());
                subset_data->m_vertex_joint_binding.resize(vertex_index_offset + vertex_count);
                scene_mesh_vertex_joint_binding *const out_vertex_joint_binding = &subset_data->m_vertex_joint_binding[vertex_index_offset];
                ...
                if (NULL != joints_accessor)
                {
                    ...
                    case cgltf_component_type_r_8u:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < joints_accessor->count; ++vertex_index)
                        {
                            ...
                            (*reinterpret_cast<uint64_t *>(&out_vertex_joint_binding[vertex_index].m_indices_xy)) = packed_vector.v;
                        }
                    }
                    break;
                    case cgltf_component_type_r_16u:
                ...
                else
                {
                    for (size_t vertex_index = 0; vertex_index < vertex_count; ++vertex_index)
                    {
                        out_vertex_joint_binding[vertex_index].m_indices_xy = 0U;
                        out_vertex_joint_binding[vertex_index].m_indices_wz = 0U;
                    }
                }
                ...
                if (NULL != weights_accessor)
                {
                    ...
                    case cgltf_component_type_r_8u:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < weights_accessor->count; ++vertex_index)
                        {
                            ...
                            out_vertex_joint_binding[vertex_index].m_weights = packed_vector_ubyten4.v;
                        }
                    }
                    break;
                    case cgltf_component_type_r_16u:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < weights_accessor->count; ++vertex_index)
                        {
                            ...
                            DirectX::PackedVector::XMUSHORTN4 packed_vector_ushortn4;
                            packed_vector_ushortn4.x = joint_weights_ushortn4[0];
                            packed_vector_ushortn4.y = joint_weights_ushortn4[1];
                            packed_vector_ushortn4.z = joint_weights_ushortn4[2];
                            packed_vector_ushortn4.w = joint_weights_ushortn4[3];

                            DirectX::XMVECTOR unpacked_vector = DirectX::PackedVector::XMLoadUShortN4(&packed_vector_ushortn4);

                            DirectX::PackedVector::XMUBYTEN4 packed_vector_ubyten4;
                            DirectX::PackedVector::XMStoreUByteN4(&packed_vector_ubyten4, unpacked_vector);
                    ...
                    case cgltf_component_type_r_32f:
                    {
                        ...
                        for (size_t vertex_index = 0; vertex_index < weights_accessor->count; ++vertex_index)
                        {
                            ...
                            DirectX::PackedVector::XMUBYTEN4 packed_vector_ubyten4;
                            DirectX::PackedVector::XMStoreUByteN4(&packed_vector_ubyten4, DirectX::XMLoadFloat4(&unpacked_vector));
                ...
                else
                {
                    for (size_t vertex_index = 0; vertex_index < vertex_count; ++vertex_index)
                    {
                        out_vertex_joint_binding[vertex_index].m_weights = 0;
                    }
                }
                ...
                if (NULL != index_accessor)
                {
                    ...
                    case cgltf_component_type_r_8u:
                    {
                        ...
                        for (size_t index_index = 0; index_index < index_accessor->count; ++index_index)
                        {
                            ...
                            subset_data->m_max_index = std::max(subset_data->m_max_index, (vertex_index_offset + static_cast<uint32_t>(*index_ubyte)));
                    ...
                    case cgltf_component_type_r_16u:
                    {
                        ...
                        for (size_t index_index = 0; index_index < index_accessor->count; ++index_index)
                        {
                            ...
                            subset_data->m_max_index = std::max(subset_data->m_max_index, (vertex_index_offset + static_cast<uint32_t>(*index_ushort)));
                    ...
                    case cgltf_component_type_r_32u:
                    {
                        ...
                        for (size_t index_index = 0; index_index < index_accessor->count; ++index_index)
                        {
                            ...
                            subset_data->m_max_index = std::max(subset_data->m_max_index, (vertex_index_offset + (*index_uint)));

                            out_indices[index_index] = (vertex_index_offset + (*index_uint));
                        }
                    }
                    break;
                    default:
                ...
                else
                {
                    ...
                    {
                        subset_data->m_max_index = std::max(subset_data->m_max_index, static_cast<uint32_t>(index_index));
    ...
    {
        for (scene_mesh_subset_data &subset : out_mesh_data->m_subsets)
        {
            std::vector<scene_mesh_vertex_joint_binding> temp_empty;
            temp_empty.swap(subset.m_vertex_joint_binding);

            assert(subset.m_vertex_joint_binding.empty());
        }
...
#include "import_asset_malloc.h"

static void *cgltf_custom_alloc(void *, cgltf_size size)
{
    return import_asset_malloc(size, alignof(size_t));
}

static void cgltf_custom_free(void *, void *ptr)
{
    import_asset_free(ptr);
}