...
static inline void import_gltf_scene_mesh_instance_asset(std::vector<cgltf_node const *> &out_mesh_instance_nodes, std::vector<DirectX::XMFLOAT4X4> &out_mesh_instance_node_world_transforms, cgltf_data const *data, cgltf_mesh const *mesh);
    ...
    for (size_t primitive_index = 0; primitive_index < mesh->primitives_count; ++primitive_index)
    {
        ...
        if (cgltf_primitive_type_triangles == primitive->type)
        {
            ...
            if (NULL != position_accessor)
            {
                ...
                import_asset_vector<uint32_t> raw_indices(index_count);
                import_asset_vector<DirectX::XMFLOAT3> raw_positions(vertex_count);
                import_asset_vector<DirectX::XMFLOAT3> raw_normals(vertex_count);
                import_asset_vector<DirectX::XMFLOAT2> raw_texcoords(vertex_count);
                import_asset_vector<DirectX::XMFLOAT4> raw_tangents(vertex_count);
                {
                ...
                import_asset_vector<DirectX::PackedVector::XMUSHORT4> raw_joint_indices((NULL != joints_accessor && NULL != weights_accessor) ? vertex_count : static_cast<size_t>(0U));
                import_asset_vector<DirectX::XMFLOAT4> raw_joint_weights((NULL != joints_accessor && NULL != weights_accessor) ? vertex_count : static_cast<size_t>(0U));
                {
                ...
                // TODO: shall we merge the primitives within different meshes
