...
public:
    ...
    void SetViewParams(DirectX::CXMVECTOR vEyePt, DirectX::CXMVECTOR vLookatPt, DirectX::CXMVECTOR vUp);
    ...
    inline void SetDrag(bool bMovementDrag, float fTotalDragTimeToZero = 0.25f)
    {
        m_bMovementDrag = bMovementDrag;
        m_fTotalDragTimeToZero = fTotalDragTimeToZero;
    }
    inline void SetEnableYAxisMovement(bool bEnableYAxisMovement) { m_bEnableYAxisMovement = bEnableYAxisMovement; }
    inline void SetEnablePositionMovement(bool bEnablePositionMovement) { m_bEnablePositionMovement = bEnablePositionMovement; }
    inline void SetClipToBoundary(bool bClipToBoundary, DirectX::XMFLOAT3 *pvMinBoundary, DirectX::XMFLOAT3 *pvMaxBoundary)
    {
    ...
    inline void SetScalers(float fRotationScaler = 1.0f, float fMoveScaler = 5.0f)
    {
        m_fRotationScaler = fRotationScaler;
        m_fMoveScaler = fMoveScaler;
    }
    inline void SetNumberOfFramesToSmoothMouseData(int nFrames)
    {
        if (nFrames > 0)
            m_fFramesToSmoothMouseData = (float)nFrames;
    }
    inline void SetResetCursorAfterMove(bool bResetCursorAfterMove) { m_bResetCursorAfterMove = bResetCursorAfterMove; }
...
protected:
    ...
    inline bool IsKeyDown(uint8_t key) const { return ((key & KEY_IS_DOWN_MASK) == KEY_IS_DOWN_MASK); }

    DirectX::XMVECTOR XM_CALLCONV ConstrainToBoundary(DirectX::FXMVECTOR v)
    {
